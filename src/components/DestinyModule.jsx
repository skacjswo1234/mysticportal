        ...
        <div className="w-full h-full flex flex-col p-6 fade-in overflow-y-auto">
            ...
            {step === 0 && (
                <div className="glass-box">
                    <p className="mb-6 text-base text-gray-100">생년월일에 숨겨진<br/><strong className="text-yellow-300">비밀 코드</strong>를 해독해줄게!</p>
            ...
            {step === 1 && (
                <div className="flex flex-col items-center justify-center h-80">
                    ...
                    <p className="mt-6 text-pink-200 font-cute text-2xl">별들의 속삭임을 듣는 중...</p>
                </div>
            )}
            {step === 2 && res && (
                <div className="w-full flex flex-col items-center pb-10">
                    <div className="card-frame mb-8">
                        ...
                        <div className="text-8xl mb-6 pt-10 animate-bounce">{res.i}</div>
                        <div className="text-4xl text-yellow-300 font-bold font-eng mb-2 text-center">{res.k}</div>
                    </div>
                    <div className="glass-box text-left">
                        <h3 className="text-xl font-bold text-yellow-300 mb-2 flex items-center gap-2">
                            <IconStar/> {name}님의 운명
                        </h3>
                        <p className="text-lg leading-relaxed font-light">{res.m}</p>
                    </div>
                    <button onClick={()=>setStep(0)} className="text-white/60 text-base underline hover:text-white">다시 하기</button>
                </div>
            )}
        </div>
    );
